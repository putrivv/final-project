        ...
        {ratedMovies.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {ratedMovies.map((movie) => (
              <div
                key={movie.id}
                className="bg-white dark:bg-gray-800 rounded-lg shadow-xl transition-transform transform hover:scale-105 overflow-hidden"
              >
                <img
                ...
                />
                <div className="p-4">
                  <h2 className="text-xl text-gray-800 dark:text-gray-200 font-semibold mb-2">
                    {movie.title}
                  </h2>
                  <div className="flex items-center mb-3">
                    ...
                    {[...Array(5)].map((_, index) => (
                      <FaStar
                        key={index}
                        className={`h-5 w-5 ${
                          index < movie.rating ? 'text-yellow-400' : 'text-gray-400'
                        }`}
                      />
                    ))}
                  ...
                  <button
                    ...
                    className="mt-3 w-full bg-red-600 hover:bg-red-800 text-white font-semibold py-2 rounded-md transition-transform duration-300 shadow-md"
                  >
                    Delete
                  </button>
                </div>
